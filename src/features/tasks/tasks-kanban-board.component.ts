...
export class TasksKanbanBoardComponent {
  ...
  isCardDragDisabled(task: Task, column: KanbanColumn): boolean {
    if (column.status === 'done' || column.status === 'cancelled') return true;
  ...
  /**
   * Handler de drop de CDK. No usa `transferArrayItem`: solo deriva origen/destino
   * de los IDs de los `cdkDropList` y emite el intent — el contenedor decide el
   * overlay optimista y el revert.
   */
